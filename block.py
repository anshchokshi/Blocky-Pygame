...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if len(self.children) == 4:
            self.position = position
            temp = self._children_positions()
            count = 0

            while count < 4:
                self.children[count].position = temp[count]
                count += 1
            for item in self.children:
                item._update_children_positions(item.position)
        else:
            self.position = position
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            ...
            while count < 4:
                child = Block(child_position[count], self._child_size(),
            ...
            self.colour = None
            for item in self.children:
                item._helper_smash(self.max_depth)

            return True

    def _helper_smash(self, max_depth: int) -> bool:
        """
        ...
        if not self.smashable():
            ...
            if rand < temp:
                ...
                for item in self.children:
                    item._helper_smash(max_depth)
                return True
            else:
                self.colour = x[random.randint(0, 3)]
                return False
    ...
    def combine(self) -> bool:
        ...
        if self.level != (self.max_depth - 1) and len(self.children) == 0:
            ...
            for child in self.children:
                if child.colour == x[0]:
                    count_b += 1
                elif child.colour == x[1]:
                    count_r += 1
                elif child.colour == x[2]:
                    count_g += 1
                elif child.colour == x[3]:
                    count_y += 1
            lst = [count_b, count_r, count_g, count_y]
            for item in range(4):
                if lst[item] > 2:
                    majority = x[item]
            if majority is None:
                if count_y == count_g == count_r == count_b:
                    return False
                if count_b == 2 and count_r < 2 and count_y < 2 \
                        and count_g < 2:
                    majority = x[0]
                elif count_r == 2 and count_b < 2 and count_y < 2 \
                        and count_g < 2:
                    majority = x[1]
                elif count_y == 2 and count_r < 2 and count_b < 2 \
                        and count_g < 2:
                    majority = x[3]
                elif count_g == 2 and count_r < 2 and count_y < 2 \
                        and count_b < 2:
                    majority = x[2]
            if majority is None:
                return False
